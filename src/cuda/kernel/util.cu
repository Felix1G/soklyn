...
template<typename T> __device__ inline void softmax_kernel(T* out, const uint32_t m, const uint32_t n) {
...
template<typename T> __device__ inline f32_t norm_derivative(
    const T* norm_rstd, const T* centered_out, const T* prenorm_out,
    const f32_t err_delta, const f32_t n_w, f32_t& dNorm_w_acc, f32_t& dNorm_b_acc,
    const uint32_t norm, const uint32_t b_idx, const f32_t inv_n, const f32_t inv_m
) {
    f32_t d_prenorm = err_delta;
    const f32_t rstd = static_cast<f32_t>(norm_rstd[b_idx]);
...
template<typename T> __device__ inline void dev_write_norm_gradients(
    f32_t* d_prenorm_out, f32_t* dNorm_w, f32_t* dNorm_b,
    const T* norm_rstd, const T* centered_out, const T* prenorm_out,
    const f32_t err_delta, const f32_t n_w, const f32_t loss_scale,
    ...
    const f32_t d_prenorm = norm_derivative<T>(
        norm_rstd, centered_out, prenorm_out, err_delta, n_w,
