...
__device__ __forceinline__ void dev_conv_read_to_shared_memory(
    ...
    const uint32_t tid, const uint32_t total_tile_elems, const uint32_t threads_per_block,
    const uint32_t tile_stride, const uint32_t tile_w, const uint32_t in_h_start, const uint32_t in_w_start,
    const uint32_t ih, const uint32_t iw, const uint32_t pad, const uint32_t pad_mode,
    const uint32_t act = 0, const f32_t leaky_relu_coeff = 0.0f, T* __restrict__ postact_features = nullptr
) {
