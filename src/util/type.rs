...
pub trait PrecisionType:
    private::Sealed + DeviceRepr + Copy + 'static + ValidAsZeroBits + Pod + NoUninit + Debug
{
    fn zero() -> Self;
...
impl<T: PrecisionType, U: PrecisionType, K: TensorContainerType>
    CastPrecision<Tensor<U, K::WithPrecision<U>>> for Tensor<T, K>
where
    ...
    <K::WithPrecision<U> as TensorContainerType>::ShapeArray: AsRef<[usize]>,
{
    ...
    fn cast(self, context: &GpuContext) -> Result<Tensor<U, K::WithPrecision<U>>, Error> {
        let tensor = Tensor::<U, K::WithPrecision<U>>::zeros(
            context,
            &<K::WithPrecision<U> as TensorContainerType>::shape_from_slice(
                self.get_shape().as_ref(),
            ),
        )?;
